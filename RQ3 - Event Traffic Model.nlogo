;;Yang Zhou, April 2021
;;Some of the codes in this program are from the NetLogo Traffic Grid model (Wilensky, 2003)
;;I did the following changes to the original model:

;;1. Changed the geometry of the map to look like NYC, including teh directions of the maps, and added show map function.
;;2. Added different dispaly charts to track the changes caused by events, as shown in the interface.
;;3. Added a destination house to the enviornment, and added a function to have events that attracts additional cars to the system.
;;4. Implemented the behaviors of the event cars, they try to approach the destination, drop off, and then leave the system.

;;These procudures are added by me:

;;setup-taxi (taxis attracted by events)
;;event-taxi-arrivals (add event taxis to the system)
;;turnleft (taxi make turns if they get closer to destination after turning)
;;turnright (taxi make turns if they get closer to destination after turning)
;;turnleft2 (used after drop-off, taxi make random turns, until they arrive at the edge of map and leave the system)
;;turnright2 (used after drop-off, taxi make random turns, until they arrive at the edge of map and leave the system)

;;The rest of the procedures are from the original model, I have also modified the go procedure to call the procedures I wrote.
...
to setup
  ...
  if show-real-map[
    import-drawing "netlogo model map.png"]
...
to go
  ...
  [
    if taxi-arrive-per-sec >= 1[
        create-cars taxi-arrive-per-sec [setup-taxi]]
    if taxi-arrive-per-sec < 1 and random-float 1 <= taxi-arrive-per-sec [
        create-cars 1 [setup-taxi]]
  ...
  ;;taxi behaviors - arrive at destination
  ask cars with [event-taxi? = true ] [

    if distance destination <= 5 [set dropped-off? true] ;;drop off customers
    if dropped-off? = true and [edge?] of patch-ahead 1 = true [die]  ;;leave system if dropped off = true and at edge
    ifelse dropped-off? = false [turnright turnleft][turnright2 turnleft2] ;; turn to turn

  ]

  ;;special intersection where cars can only turn right
...
to turnright
  ...
  ;; check if it gets closer to destination after turning, turn if true
  if can-turn-right = 1 [
    move-to patch-here
...
;;after drop off, taxis wander on the map to try and find new customers, until they are at the edge of the map, then they leave the system
to turnright2
  let can-turn-right 0
  if member? patch-here turnpoint [
    if direction = 1 and member? [pycor] of patch-here rightlist  [set can-turn-right 1]
...
The map approximately simulate an area in New York City near Madison Sqaure Garden. Turn on the real map option to show map. 
...
Press the event-taxi-arrivals button to start an event, and event taxis will be arriving during the following hour. Event taxis can make turns, if they get closer to the destination after makeing the turn. Each tick here is equal to 1 second. If you wish to have taxis arrive during more then 1 hour, press event-taxi-arrivals again at the end of the first hour.
